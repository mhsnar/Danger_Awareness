...
class RobotPDTrackingController:
    def __init__(self):
        ...
        self.pose_sub = rospy.Subscriber('/natnet_ros/Locobot/pose', PoseStamped, self.pose_callback)
    ...
    def pose_callback(self, msg):
        ...
        self.current_yaw = 2.67+self.quaternion_to_euler(orientation_q)
        ...
        if self.prev_x is not None and self.prev_y is not None and self.prev_yaw is not None:
            ...
            # Stop the robot if error is within tolerance
            if pos_error < self.error_tolerance:
                cmd_msg = Twist()
