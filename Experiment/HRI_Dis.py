    ...
    def odom_callback(self, msg):
        ...
        if dt >= 0.2:
            if self.prev_x is not None and self.prev_y is not None and self.prev_yaw is not None:
                ...
                # Compute position error to the tracked human
                pos_error = math.sqrt((self.human_position.x - self.current_x) ** 2 + (self.human_position.y - self.current_y) ** 2)

                # Compute desired angle to the tracked human
                desired_theta = math.atan2(self.human_position.y - self.current_y, self.human_position.x - self.current_x)
